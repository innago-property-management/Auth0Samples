...
public interface IUserService
{
    ...
    ///     metadata key-value pairs, or null if no matches are found.
    /// </returns>
    ITask<IReadOnlyDictionary<string, IReadOnlyDictionary<string, string?>?>?> GetUsersMetadataByNameAndEmailAndPhoneFragment(
    ...
    ///     metadata key-value pairs, or null if no matches are found.
    /// </returns>
    ITask<IReadOnlyDictionary<string, IReadOnlyDictionary<string, string?>?>?> CheckPhoneExistsOnAuth0(
        string searchTerm,
        IEnumerable<string>? keys,
        CancellationToken cancellationToken);
    /// <summary>
